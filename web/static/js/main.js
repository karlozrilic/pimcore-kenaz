...
var VIDEO_TESTEMONIALS_BASE_URL = "http://example.com/video-testimonials";
var VIDEO_TESTEMONIALS_LIST_URL = "http://example.com/all-video-testimonials";
$(function () {
  var homePage = $('#home-page');
  var categoryPage = $('#category-page');
  var postPage = $('#post-page');
  var testemonialsPage = $('#all-testemonials');
  var numberOfTestemonials = $('.video-testemonial').length;
        ...
        progress: function progress(animation, _progress, remainingMs) {
          if (remainingMs <= 200) {
            $(testemonials).each(function (index, element) {
              $(element).removeClass("z-ind-".concat(index + 1)).addClass("z-ind-".concat(index == 0 ? numberOfTestemonials : index));
            });
          }

          if (remainingMs <= 800) {
            $(testemonials).each(function (index, element) {
              $(element).removeClass("tab-".concat(index + 1)).addClass("tab-".concat(index == 0 ? numberOfTestemonials : index));
            });
          }
        },
        done: function done() {
  ...
  if (testemonialsPage.exists()) {
    var filters = $(".filters");
    var testemonialsList = $(".list-of-testemonials");
    var filterList = [];
    ...
    var handleFilterChange = function handleFilterChange(filterList) {
      filter(filterList).then(function (data) {
        ...
        data.video_testimonials.forEach(function (testemonial) {
    ...
    var filter = function filter(filterList) {
      return axios__WEBPACK_IMPORTED_MODULE_0___default().get(VIDEO_TESTEMONIALS_BASE_URL, {
        params: {
          json: true,
          categories: filterList
        }
      }).then(function (response) {
        var url = axios__WEBPACK_IMPORTED_MODULE_0___default().getUri({
          url: VIDEO_TESTEMONIALS_LIST_URL,
          params: {
            categories: filterList
          }
        });
        var values = Object.values(response.data.filter_categories);
    ...
    var makeTestemonialTemplate = function makeTestemonialTemplate(_ref) {
      var author_name = _ref.author_name,
          author_surname = _ref.author_surname,
    ...
    var makeTestemonialFiltersTemplate = function makeTestemonialFiltersTemplate(_ref2, filterCategories) {
      var _ref3 = _slicedToArray(_ref2, 2),
          categoryId = _ref3[0],
          categoryName = _ref3[1];

      return "\n                <input class=\"input\" type=\"checkbox\" id=\"".concat(categoryName.toLowerCase(), "\" name=\"").concat(categoryName.toLowerCase(), "\" value=").concat(categoryId, " ").concat(filterCategories.includes(categoryId) && "checked", ">\n                <label for=\"").concat(categoryName.toLowerCase(), "\">").concat(categoryName, "</label><br>\n            ");
    };
